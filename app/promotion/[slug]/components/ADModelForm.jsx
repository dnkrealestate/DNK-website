...
export const ADModelForm = ({ onFormSubmit, promotionData }) => {
  ...
  const addLeadtobitrix = async (name, email, phone, city) => {
    ...
    const leadData = {
      fields: {
        TITLE: `${promotionData.projectName} Website Lead`,
        NAME: name,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
     ...
     track(`Contact form submitted ${promotionData.projectName}`, {
       track: `name: ${fullName},
              phone: ${phoneNumber},
              email: ${email},
              city: ${city},
    ...
    <form onSubmit={handleSubmit}>
      <div className="flex flex-col justify-between mx-0 md:mx-[10px] mt-3 md:mt-0">
        ...
        <button
          ...
          disabled={loading}
          className="bg-white text-black hover:bg-[var(--themeColor)] hover:text-white w-full p-[10px] mt-6 rounded flex justify-center"
        >
          {loading ? (
            <div style={{ "--themeColor": promotionData.themeColor }} className="loader w-6 h-6 border-t-2 border-b-2 border-[var(--themeColor)] rounded-full animate-spin" />
          ) : (
            "Submit"
          )}
        </button>
        ...
        <div className="flex items-center justify-center mt-4">
          ...
          <div onClick={() => { track("WhatsApp Button Click Contact Form", {
                            page: `${promotionData.projectName}`,
                            button: "WhatsApp Button Contact Form",
                            whatsapp: "Msg to Waseem",
                            message:
                              `Hello, could you please provide more insights into the project ${promotionData.projectName}?`,
                          });}}>
            <a
              href={`https://wa.me/+971543049309?text=Hey, Please share details of ${promotionData.projectName}.`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center"
            >
              <FaWhatsapp className="text-[#6B9B2D] text-[1.3rem] ml-2 group-hover:!text-[#6B9B2D]" />
              <p className="mb-0 text-[#6B9B2D] group-hover:!text-[#6B9B2D]">
                WhatsApp
              </p>
            </a>
          </div>
